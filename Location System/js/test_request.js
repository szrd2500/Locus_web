$(function () {
    ...
    function setup() {
        ...
        document.getElementById("canvas").addEventListener("mousemove", handleMouseMove, false); //滑鼠在畫布中移動的座標
        document.getElementById("canvas").addEventListener("touchstart", handleMobileTouch, { //手指點擊畫布中座標，跳出tag的訊息框
            passive: true
        });
        document.getElementById("canvas").addEventListener("mousewheel", handleMouseWheel, { //畫布縮放
            passive: true
        });
        document.getElementById("canvas").addEventListener("DOMMouseScroll", handleMouseWheel, false); // 畫面縮放(for Firefox)
        document.getElementById("canvas").addEventListener('click', handleMouseClick, false); //點擊地圖上的tag，跳出tag的訊息框
        ...
        canvas.getMaps();
        mapArray = canvas.mapArray;
        var html = "";
        for (i in mapArray) {
            html += "<li><input type=\"button\" id=\"map_btn_" + MapList[i].map_id + "\" " +
                "value=\"" + MapList[i].map_name + "\" onclick=\"addMapTab(\'" + MapList[i].map_id +
                "\',\'" + MapList[i].map_name + "\')\" /></li>";
        }
        ...
        xmlHttp.onreadystatechange = function () {
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
                ...
                for (var i = 0; i < revObj.length; i++) {
                    ...
                    if (revObj[i].tag_id == locating_id) {
                        focus_data = {
                            id: revObj[i].tag_id,
