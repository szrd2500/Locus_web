...
function reDrawTag(dctx) {
    ...
    for (i = 0; i < time; i++) {
        if (i == 0)
            ...
            drawArrow(dctx, array[i - 1].x, array[i - 1].y, array[i].x, array[i].y, 30, 8, 2, '#ff5500');
...
function drawNextTime() {
    ...
    if (isContinue && array.length > 0) {
        if (time == 0) {
            ...
            drawArrow(ctx, array[time - 1].x, array[time - 1].y, array[time].x, array[time].y, 30, 8, 2, '#ff5500');
