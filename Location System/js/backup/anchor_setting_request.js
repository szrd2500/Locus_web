    ...
    if (ip_arr_len > 0) {
        ...
        if (is_network) {
            ...
            var set_mode = "",
                ...
                gateway_address = [],
                client_ip = [];
            if (document.getElementsByName("network_setting_mode")[0].checked)
                set_mode = "DHCP";
            else
                set_mode = "Fixed IP";
            for (i = 1; i < 5; i++) {
        ...
        if (is_basic) {
            ...
            var basicArray = {
                ...
                "Value": {
                    "Connected_IP": connect_ip_array,
                    "Sent_cycle": [document.getElementById("sent_cycle").value],
...
function read_network(requestArray) {
    ...
    xmlHttp.open("POST", "Command", true); //Device_setting
    xmlHttp.setRequestHeader("Content-type", "application/json");
    xmlHttp.send(JSON.stringify(requestArray));
}
...
function RF_setting_read() {
    ...
    if (ip_arr_len > 0) {
        ...
        xmlHttp.open("POST", "Command", true); //DeviceNetworkSetting
        xmlHttp.setRequestHeader("Content-type", "application/json");
        xmlHttp.send(JSON.stringify(requestArray));
    } else {
        alert("請連線至少一台裝置");
